...
object UpdateExpression {
  ...
  def collectFailures(action: Action[_]): List[String] =
    action match {
      case Action.Failure(msg)   => List(msg)
      case Action.Actions(chunk) => chunk.toList.flatMap(collectFailures)
      case _                     => Nil
    }
  ...
  sealed trait Action[-From] { self =>
    ...
    def render: AliasMapRender[String] = {
      ...
      self match {
        ...
        case Action.Failure(_)                => AliasMapRender.succeed("")
      }
    }
    ...
    def collectActions(actions: Chunk[RenderableAction[_]]) = {
      ...
      actions.foreach {
        ...
        case _: Action.Failure[_]           => () // guarded by validateAction in the interpreter
      }
  ...
  object Action {
    ...
    private[dynamodb] final case class Failure[-From](message: String) extends RenderableAction[From] {
      override def miniRender: AliasMapRender[String] = AliasMapRender.succeed("")
    }
  }
