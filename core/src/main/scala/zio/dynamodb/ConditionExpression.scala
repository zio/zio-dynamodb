    ...
    self match {
      ...
      case f: Failure[_]               =>
        throw new IllegalStateException(
          s"ConditionExpression.Failure reached render — validation pass was skipped: ${f.message}"
        )
    }
...
object ConditionExpression {
  ...
  object Operand {
    ...
    private[dynamodb] final case class Size[-From, To](path: ProjectionExpression[From, To]) extends Operand[From, Long]
  ...
  // Sentinel for expressions that could not be converted (e.g. unrecognised SchemaExpr variants).
  // Never sent to AWS; the interpreter collects these and raises F.fail before building a request.
  private[dynamodb] final case class Failure[-From](message: String) extends ConditionExpression[From]

  // Walks the expression tree and returns all Failure messages, accumulating across And/Or/Not branches.
  def collectFailures(ce: ConditionExpression[_]): List[String] =
    ce match {
      case f: Failure[_] => List(f.message)
      case a: And[_]     => collectFailures(a.left) ++ collectFailures(a.right)
      case o: Or[_]      => collectFailures(o.left) ++ collectFailures(o.right)
      case n: Not[_]     => collectFailures(n.exprn)
      case _             => Nil
    }

  def collectFailures(ce: Option[ConditionExpression[_]]): List[String] =
    ce.toList.flatMap(collectFailures)
}
