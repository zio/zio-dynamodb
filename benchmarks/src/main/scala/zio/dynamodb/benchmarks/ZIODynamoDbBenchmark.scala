...
private[benchmarks] final class ZIODynamoDbBenchmark private (
  runtime: Runtime.Scoped[TestDynamoDBExecutor with DynamoDBExecutor]
) {
  ...
  def tearDown(): Unit = Unsafe.unsafe(implicit unsafe => runtime.unsafe.shutdown())
}
object ZIODynamoDbBenchmark {
  def unsafeMake(): ZIODynamoDbBenchmark = {
    val runtime = Unsafe.unsafe(implicit unsafe =>
      Runtime.unsafe.fromLayer(DynamoDBExecutor.test(Person.tableName -> Person.idColumnName))
    )
