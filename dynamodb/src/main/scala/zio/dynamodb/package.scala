...
package object dynamodb {
  ...
  def batchReadItemFromStream[R, A](
    ...
    pk: A => PrimaryKey
  ): ZStream[R with Has[DynamoDBExecutor] with Clock, Throwable, (A, Item)] =
    stream
  ...
  def batchReadFromStream[R, A, B: Schema](
    ...
    stream: ZStream[R, Throwable, A],
    mPar: Int = 10
  )(pk: A => PrimaryKey): ZStream[R with Has[DynamoDBExecutor] with Clock, Throwable, (A, B)] =
    batchReadItemFromStream(tableName, stream, mPar)(pk).mapM {
      case (a, item) =>
