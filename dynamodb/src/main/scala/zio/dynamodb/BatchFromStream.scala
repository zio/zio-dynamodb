...
object BatchFromStream {
  ...
  )(f: A => DynamoDBQuery.Write[Unit]): ZStream[DynamoDBExecutor with R, Exception, Unit] =
  ...
  def batchReadFromStream[R, A](
    ...
    pk: A => PrimaryKey
  )(f: List[Option[Item]] => ZIO[R, Exception, Unit]): ZStream[R with DynamoDBExecutor, Exception, Unit] =
    stream.grouped(100).mapMPar(mPar) { chunk =>
