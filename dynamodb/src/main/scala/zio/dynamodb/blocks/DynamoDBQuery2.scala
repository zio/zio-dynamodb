...
// import zio.dynamodb.DynamoDBQuery2.BatchGetItem.TableGet
// import zio.dynamodb.DynamoDBQuery2.BatchWriteItem.{ Delete, Put }
// import zio.dynamodb.DynamoDBQuery2.{
//   batched,
//   parallelize,
//   Absolve,
//   BatchGetItem,
...
// sealed trait DynamoDBQuery2[-In, +Out] { self =>

//   final def <*[In1 <: In, B](that: DynamoDBQuery2[In1, B]): DynamoDBQuery2[In1, Out] = zipLeft(that)

//   final def *>[In1 <: In, B](that: DynamoDBQuery2[In1, B]): DynamoDBQuery2[In1, B] = zipRight(that)

//   final def <*>[In1 <: In, B](that: DynamoDBQuery2[In1, B]): DynamoDBQuery2[In1, (Out, B)] = self zip that

//   def execute: ZIO[DynamoDBExecutor, DynamoDBError, Out] = {
//     val validateBatching = self match {
//       case DynamoDBQuery2.Map(Zip(_, _, _, validateBatching), _) =>
//         validateBatching
//       case _                                                    =>
//         false
//     }

//     val (constructors: Chunk[DynamoDBQuery2.Constructor[In, Any]], assembler: Function1[Chunk[Any], Out]) = parallelize(
//       self
//     )
//     val (
//       indexedConstructors: Chunk[(DynamoDBQuery2.Constructor[In, Any], Int)],
...
//   final def indexName(indexName: String): DynamoDBQuery2[In, Out] =
//     self match {
//       case Zip(left, right, zippable, _) => Zip(left.indexName(indexName), right.indexName(indexName), zippable)
//       case Map(query, mapper)            => Map(query.indexName(indexName), mapper)
//       case Absolve(query)                => Absolve(query.indexName(indexName))
//       case q: ScanAll                    =>
//         q.copy(indexName = Some(IndexName(indexName))).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case q: ScanSome                   =>
//         q.copy(indexName = Some(IndexName(indexName))).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case q: QueryAll                   =>
//         q.copy(indexName = Some(IndexName(indexName))).asInstanceOf[DynamoDBQuery2[In, Out]]
...
//   final def capacity(capacity: ReturnConsumedCapacity): DynamoDBQuery2[In, Out] =
//     self match {
//       case Zip(left, right, zippable, _) => Zip(left.capacity(capacity), right.capacity(capacity), zippable)
//       case Map(query, mapper)            => Map(query.capacity(capacity), mapper)
//       case Absolve(query)                => Absolve(query.capacity(capacity))
//       case g: GetItem                    =>
//         g.copy(capacity = capacity).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case b: BatchGetItem               =>
//         b.copy(capacity = capacity).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case b: BatchWriteItem             =>
//         b.copy(capacity = capacity).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case q: ScanAll                    =>
//         q.copy(capacity = capacity).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case q: ScanSome                   =>
//         q.copy(capacity = capacity).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case q: QueryAll                   =>
//         q.copy(capacity = capacity).asInstanceOf[DynamoDBQuery2[In, Out]]
...
//         q.copy(capacity = capacity).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case m: PutItem                    =>
//         m.copy(capacity = capacity).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case m: UpdateItem                 =>
//         m.copy(capacity = capacity).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case m: DeleteItem                 =>
//         m.copy(capacity = capacity).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case t: Transaction[_]             =>
//         t.copy(capacity = capacity).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case _                             => self
//     }

//   final def consistency(consistency: ConsistencyMode): DynamoDBQuery2[In, Out] =
//     self match {
//       case Zip(left, right, zippable, _) => Zip(left.consistency(consistency), right.consistency(consistency), zippable)
//       case Map(query, mapper)            => Map(query.consistency(consistency), mapper)
//       case Absolve(query)                => Absolve(query.consistency(consistency))
//       case g: GetItem                    =>
//         g.copy(consistency = consistency).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case q: ScanAll                    =>
//         q.copy(consistency = consistency).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case q: ScanSome                   =>
//         q.copy(consistency = consistency).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case q: QueryAll                   =>
//         q.copy(consistency = consistency).asInstanceOf[DynamoDBQuery2[In, Out]]
...
//    */
//   def returns(returnValues: ReturnValues): DynamoDBQuery2[In, Out] =
//     self match {
//       case Zip(left, right, zippable, _) => Zip(left.returns(returnValues), right.returns(returnValues), zippable)
//       case Map(query, mapper)            => Map(query.returns(returnValues), mapper)
//       case Absolve(query)                => Absolve(query.returns(returnValues))
//       case p: PutItem                    =>
//         p.copy(returnValues = returnValues).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case u: UpdateItem                 =>
//         u.copy(returnValues = returnValues).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case d: DeleteItem                 =>
//         d.copy(returnValues = returnValues).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case _                             => self
//     }

//   def where[B](conditionExpression: ConditionExpression[B])(implicit ev: CanWhere[B, Out]): DynamoDBQuery2[In, Out] = {
//     val _ = ev
//     self match {
//       case zip @ Zip(left, right, zippable, _) =>
//         Zip(
...
//       case p: PutItem                          =>
//         p.copy(conditionExpression = Some(conditionExpression)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case u: UpdateItem                       =>
//         u.copy(conditionExpression = Some(conditionExpression)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case d: DeleteItem                       =>
//         d.copy(conditionExpression = Some(conditionExpression)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case _                                   => self
//     }
//   }

//   def metrics(itemMetrics: ReturnItemCollectionMetrics): DynamoDBQuery2[In, Out] =
//     self match {
//       case Zip(left, right, zippable, _) => Zip(left.metrics(itemMetrics), right.metrics(itemMetrics), zippable)
//       case Map(query, mapper)            => Map(query.metrics(itemMetrics), mapper)
//       case Absolve(query)                => Absolve(query.metrics(itemMetrics))
//       case p: PutItem                    =>
//         p.copy(itemMetrics = itemMetrics).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case u: UpdateItem                 =>
//         u.copy(itemMetrics = itemMetrics).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case d: DeleteItem                 =>
//         d.copy(itemMetrics = itemMetrics).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case t: Transaction[_]             =>
//         t.copy(itemMetrics = itemMetrics).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case _                             => self
//     }

//   def startKey(exclusiveStartKey: LastEvaluatedKey): DynamoDBQuery2[In, Out] =
//     self match {
...
//       case s: ScanSome                   => s.copy(exclusiveStartKey = exclusiveStartKey).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case s: ScanAll                    => s.copy(exclusiveStartKey = exclusiveStartKey).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case s: QuerySome                  => s.copy(exclusiveStartKey = exclusiveStartKey).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case s: QueryAll                   => s.copy(exclusiveStartKey = exclusiveStartKey).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case _                             => self
//     }
...
//    */
//   def filter[B](filterExpression: FilterExpression[B])(implicit ev: CanFilter[B, Out]): DynamoDBQuery2[In, Out] = {
//     val _ = ev
//     self match {
//       case zip @ Zip(left, right, zippable, _) =>
//         Zip(
...
//       case s: ScanSome                         => s.copy(filterExpression = Some(filterExpression)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case s: ScanAll                          => s.copy(filterExpression = Some(filterExpression)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case s: QuerySome                        => s.copy(filterExpression = Some(filterExpression)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case s: QueryAll                         => s.copy(filterExpression = Some(filterExpression)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case _                                   => self
//     }
//   }
...
//    */
//   def parallel(n: Int): DynamoDBQuery2[In, Out] =
//     self match {
//       case Zip(left, right, zippable, _) => Zip(left.parallel(n), right.parallel(n), zippable)
//       case Map(query, mapper)            => Map(query.parallel(n), mapper)
//       case Absolve(query)                => Absolve(query.parallel(n))
//       case s: ScanAll                    => s.copy(totalSegments = n).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case _                             => self
//     }
...
//           )
//         ).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case _                             => self
//     }
...
//   def selectAllAttributes: DynamoDBQuery2[In, Out]          = select(Select.AllAttributes)
//   def selectAllProjectedAttributes: DynamoDBQuery2[In, Out] = select(Select.AllProjectedAttributes)
//   def selectSpecificAttributes: DynamoDBQuery2[In, Out]     = select(Select.SpecificAttributes)
//   def selectCount: DynamoDBQuery2[In, Out]                  = select(Select.Count)

//   /**
//    * Adds a KeyConditionExpr to a DynamoDBQuery2. Example:
//    * {{{
...
//    * }}}
//    */
//   def whereKey[From](keyConditionExpression: KeyConditionExpr[From]): DynamoDBQuery2[In, Out] =
//     self match {
//       case Zip(left, right, zippable, _) =>
...
//       case s: QuerySome =>
//         s.copy(keyConditionExpr = Some(keyConditionExpression)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case s: QueryAll  =>
//         s.copy(keyConditionExpr = Some(keyConditionExpression)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case _            => self
//     }

//   def withRetryPolicy(retryPolicy: Schedule[Any, Throwable, Any]): DynamoDBQuery2[In, Out] =
//     self match {
//       case Zip(left, right, zippable, _) =>
...
//       case bw: BatchWriteItem            => bw.copy(retryPolicy = Some(retryPolicy)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case bg: BatchGetItem              => bg.copy(retryPolicy = Some(retryPolicy)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case p: PutItem                    => p.copy(retryPolicy = Some(retryPolicy)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case d: DeleteItem                 => d.copy(retryPolicy = Some(retryPolicy)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case g: GetItem                    => g.copy(retryPolicy = Some(retryPolicy)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case _                             => self
//     }

//   def sortOrder(ascending: Boolean): DynamoDBQuery2[In, Out] =
//     self match {
//       case Zip(left, right, zippable, _) => Zip(left.sortOrder(ascending), right.sortOrder(ascending), zippable)
//       case Map(query, mapper)            => Map(query.sortOrder(ascending), mapper)
//       case Absolve(query)                => Absolve(query.sortOrder(ascending))
//       case qs: QuerySome                 => qs.copy(ascending = ascending).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case qa: QueryAll                  => qa.copy(ascending = ascending).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case _                             => self
//     }

//   def withClientRequestToken(token: String): DynamoDBQuery2[In, Out] =
//     self match {
//       case Zip(left, right, zippable, _) =>
...
//   final def map[B](f: Out => B): DynamoDBQuery2[In, B] = DynamoDBQuery2.Map(self, f)

//   final def zip[In1 <: In, B](that: DynamoDBQuery2[In1, B])(implicit
//     z: Zippable[Out, B]
//   ): DynamoDBQuery2[In1, z.Out] =
//     DynamoDBQuery2.Zip[Out, B, z.Out](self, that, z, validateBatching = false)

//   private[dynamodb] final def zip[In1 <: In, B](that: DynamoDBQuery2[In1, B], validateBatching: Boolean)(implicit
//     z: Zippable[Out, B]
//   ): DynamoDBQuery2[In1, z.Out] =
//     DynamoDBQuery2.Zip[Out, B, z.Out](self, that, z, validateBatching)

//   final def zipLeft[In1 <: In, B](that: DynamoDBQuery2[In1, B]): DynamoDBQuery2[In1, Out] = (self zip that).map(_._1)

//   final def zipRight[In1 <: In, B](that: DynamoDBQuery2[In1, B]): DynamoDBQuery2[In1, B] = (self zip that).map(_._2)

//   final def zipWith[In1 <: In, B, C](that: DynamoDBQuery2[In1, B])(
//     f: (Out, B) => C
//   ): DynamoDBQuery2[In1, C] =
//     self.zip(that).map(f.tupled)

//   private[dynamodb] final def zipWithValidateBatching[In1 <: In, B, C](that: DynamoDBQuery2[In1, B])(
//     f: (Out, B) => C
//   ): DynamoDBQuery2[In1, C] =
//     self.zip(that, validateBatching = true).map(f.tupled)

//   private def select(select: Select): DynamoDBQuery2[In, Out] =
//     self match {
//       case Zip(left, right, zippable, _) => Zip(left.select(select), right.select(select), zippable)
//       case Map(query, mapper)            => Map(query.select(select), mapper)
//       case Absolve(query)                => Absolve(query.select(select))
//       case s: ScanSome                   => s.copy(select = Some(select)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case s: ScanAll                    => s.copy(select = Some(select)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case s: QuerySome                  => s.copy(select = Some(select)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case s: QueryAll                   => s.copy(select = Some(select)).asInstanceOf[DynamoDBQuery2[In, Out]]
//       case _                             => self
//     }

//   final def transaction: DynamoDBQuery2[In, Out] = Transaction(self).asInstanceOf[DynamoDBQuery2[In, Out]]

//   final def safeTransaction: Either[DynamoDBError.TransactionError, DynamoDBQuery2[Any, Out]] = {
...
//   sealed trait Constructor[-In, +A] extends DynamoDBQuery2[In, A]
//   sealed trait Write[-In, +A]       extends Constructor[In, A]

//   def succeed[A](a: => A): DynamoDBQuery2[Any, A] = Succeed(() => a)

//   def fail(e: => DynamoDBError): DynamoDBQuery2[Any, Nothing] = Fail(() => e)

//   private[dynamodb] def absolve[A, B](query: DynamoDBQuery2[A, Either[ItemError, B]]): DynamoDBQuery2[A, B] =
//     Absolve(query)

//   def fromEither[A](or: Either[ItemError, A]): DynamoDBQuery2[Any, A] =
//     or match {
//       case Left(error)  => DynamoDBQuery2.fail(error)
//       case Right(value) => DynamoDBQuery2.succeed(value)
//     }
...
//   def forEach[In, A, B](values: Iterable[A])(body: A => DynamoDBQuery2[In, B]): DynamoDBQuery2[In, List[B]] =
//     values.reverse.foldLeft[DynamoDBQuery2[In, List[B]]](succeed(Nil)) {
//       case (queryAcc: DynamoDBQuery2[In, List[B]], a) =>
//         val bodyApplied: DynamoDBQuery2[In, B] = body(a)
//         val zip: DynamoDBQuery2[In, List[B]]   = bodyApplied.zipWithValidateBatching(queryAcc)(_ :: _)
//         zip
//     }
...
//    * `DynamoDBError.BatchError` should be provided.
//    */
//   def batch[In, A, B](values: Iterable[A])(body: A => DynamoDBQuery2[In, B]): DynamoDBQuery2[In, Chunk[B]] =
//     values.reverse.foldLeft[DynamoDBQuery2[In, Chunk[B]]](succeed(Chunk.empty)) {
//       case (queryAcc, a) =>
//         val bodyApplied: DynamoDBQuery2[In, B] = body(a)
//         val zip: DynamoDBQuery2[In, Chunk[B]]  = bodyApplied.zipWithValidateBatching(queryAcc)((el, acc) => el +: acc)
//         zip
//     }
...
//   ): DynamoDBQuery2[Any, Option[Item]] =
...
//   ): DynamoDBQuery2[From, Either[ItemError, From]] =
...
//   ): DynamoDBQuery2[From, Either[ItemError, To]] = {
...
//   ): DynamoDBQuery2[A, Either[ItemError, A]] =
...
//   def putItem(tableName: String, item: Item): DynamoDBQuery2[Any, Option[Item]] = PutItem(TableName(tableName), item)

//   def put[A: Schema](tableName: String, a: A): DynamoDBQuery2[A, Option[A]] =
...
//     action: Action[From]
//   ): DynamoDBQuery2[From, Option[From]] =
//     updateItem(tableName, primaryKeyExpr.asAttrMap)(action).map(_.flatMap(item => fromItem(item).toOption))

//   def updateItem[A](tableName: String, key: PrimaryKey)(action: Action[A]): DynamoDBQuery2[A, Option[Item]] =
//     UpdateItem(
//       TableName(tableName),
//       key,
...
//   )(
//     primaryKeyExpr: KeyConditionExpr.PrimaryKeyExpr[From]
//   ): DynamoDBQuery2[Any, Option[From]] =
...
//     tableName: String,
//     limit: Int
//   ): DynamoDBQuery2[A, (Chunk[A], LastEvaluatedKey)] =
//     DynamoDBQuery2.absolve(
...
//     tableName: String
//   ): DynamoDBQuery2[A, Stream[Throwable, A]] =
...
//     left: DynamoDBQuery2[_, A],
//     right: DynamoDBQuery2[_, B],
//     zippable: Zippable.Out[A, B, C],
//     validateBatching: Boolean = false
//   ) extends DynamoDBQuery2[Any, C] {
//     type Left  = A
//     type Right = B
//   }

//   private[dynamodb] final case class Map[A, B](query: DynamoDBQuery2[_, A], mapper: A => B)
//       extends DynamoDBQuery2[Any, B] {
//     type Old = A
//   }

//   private[dynamodb] final case class Absolve[A, B](query: DynamoDBQuery2[A, Either[ItemError, B]])
//       extends DynamoDBQuery2[A, B] {
//     type Old = Either[ItemError, B]
//   }

//   def apply[A](a: => A): DynamoDBQuery2[Any, A] = Succeed(() => a)
...
//       case Absolve(query)        =>
//         val absolved: DynamoDBQuery2[In, A] = query.map {
