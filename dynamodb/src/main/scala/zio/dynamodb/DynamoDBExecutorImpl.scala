...
case object DynamoDBExecutorImpl {
  ...
  private[dynamodb] def filterMixedTransactions[A](
    ...
    if (actions.isEmpty) Left(DynamoDBError.TransactionError.EmptyTransaction)
    else {
      val headConstructor = constructorToTransactionType(actions.head)
        .map(Right(_))
        .getOrElse(
          Left(DynamoDBError.TransactionError.InvalidTransactionActions(NonEmptyChunk(actions.head)))
        )        // TODO: Grab all that are invalid
      actions
        .drop(1) // dropping 1 because we have the head element as the base case of the fold
        .foldLeft(headConstructor: Either[DynamoDBError.TransactionError, TransactionType]) { case (acc, constructor) =>
          acc match {
  ...
  ): Either[DynamoDBError.TransactionError, TransactionType] =
    constructorToTransactionType(constructor)
      .map(t =>
        if (t == transactionType) Right(transactionType)
        else Left(DynamoDBError.TransactionError.MixedTransactionTypes)
      )
      .getOrElse(
        Left(DynamoDBError.TransactionError.InvalidTransactionActions(NonEmptyChunk(constructor)))
      )
  ...
  private def aliasMapToExpressionZIOAwsAttributeValues(
    ...
    if (aliasMap.isEmpty) None
      ...
      val map = aliasMap.map.flatMap {
        case (AliasMap.AttributeValueKey(attrVal), str) =>
  ...
  private def aliasMapToExpressionZIOAwsAttributeNames(
    aliasMap: AliasMap
  ...
  private def awsTransactPutItem(put: PutItem): ZIOAwsPut = {
    val (aliasMap, maybeConditionExpression) =
      AliasMapRender.collectAll(put.conditionExpression.map(_.render)).execute
  ...
  private def awsTransactDeleteItem(delete: DeleteItem): ZIOAwsDelete = {
    val (aliasMap, maybeConditionExpression) =
      AliasMapRender.collectAll(delete.conditionExpression.map(_.render)).execute
