    ...
    suite("live test")(
      ...
      suite("update items")(
        suite("set actions")(
          ...
          suite("if not exists")(
            testM("field does not exist with a projection expression on both sides") {
              withTemporaryTable(
                numberTable,
                tableName =>
                  for {
            ...
            testM("field does not exist using projection expression on LHS and a value on RHS") {
              withTemporaryTable(
                numberTable,
                tableName =>
                  for {
        ...
        testM("delete elements from a set") {
          withTemporaryTable(
            tableName =>
              createTable(tableName, KeySchema(id), BillingMode.PayPerRequest)(AttributeDefinition.attrDefnNumber(id)),
            tableName =>
