...
object DynamicValueGen {
  ...
  def anyPrimitiveDynamicValue[A](standardType: StandardType[A]): Gen[Sized, DynamicValue.Primitive[A]] = {
    ...
    standardType match {
      ...
      case typ: StandardType.CharType.type       => gen(typ, Gen.char)
  ...
  def anyDynamicValueOfSchema[A](schema: Schema[A]): Gen[Sized, DynamicValue] =
    schema match {
      ...
      case Schema.SetSchema(ks, _)                                                                                                                                                                 => Gen.chunkOfBounded(0, 2)(anyDynamicValueOfSchema(ks)).map(DynamicValue.Sequence)
