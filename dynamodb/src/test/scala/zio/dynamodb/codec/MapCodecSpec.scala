    ...
    suite("Map codecs")(
      suite("when encoding")(
        test("encodes map with string key natively") {
          val actual: AttributeValue = Encoder(HasMapWithStringKey.schema)(HasMapWithStringKey(Map("1" -> 1)))
        ...
        test("encodes map with non string key as sequence of Tuple2") {
          val actual: AttributeValue = Encoder(HasMapWithNonStringKey.schema)(HasMapWithNonStringKey(Map(1 -> 1)))
      ...
      suite("when decoding")(
        test("decodes map with string key natively") {
          ...
          val actual = Decoder(HasMapWithStringKey.schema)(av)
        ...
        test("decodes map with non string key") {
          ...
          val actual = Decoder(HasMapWithNonStringKey.schema)(av)
