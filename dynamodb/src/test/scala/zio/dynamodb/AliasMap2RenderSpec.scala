...
object AliasMap2RenderSpec extends ZIOSpecDefault {
  ...
  def pathSegment[From, To](pe: ProjectionExpression[From, To], path: String) = AliasMap2.PathSegment(pe, path)
  def fullPath[From, To](pe: ProjectionExpression[From, To])                  = AliasMap2.FullPath(pe)
  def avKey(av: AttributeValue): AliasMap2.Key                                = AliasMap2.AttributeValueKey(av)
  def mapOfAv(av: AttributeValue, s: String): Map[AliasMap2.Key, String]      = Map(avKey(av) -> s)
  ...
  val debug = suite("debug")(
    test("Two Sets") {
      ...
      assert(aliasMap)(equalTo(AliasMap2(map, 7))) && // TODO: Avi resolve 7
      ...
      suite("ConditionExpression")(
        suite("basic renders")(
          test("Between") {
            ...
            assert(aliasMap)(equalTo(AliasMap2(map, 3))) &&
          ...
          test("In") {
            ...
            assert(aliasMap)(equalTo(AliasMap2(map, 2))) &&
          ...
          test("AttributeExists") {
            ...
            assert(aliasMap)(equalTo(AliasMap2(map, 1))) &&
          ...
          test("And") {
            ...
            assert(aliasMap)(equalTo(AliasMap2(map, 5))) &&
