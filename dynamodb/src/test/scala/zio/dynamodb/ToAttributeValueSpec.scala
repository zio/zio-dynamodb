...
object ToAttributeValueSpec extends DefaultRunnableSpec {
  ...
  val simpleAttrMapSuite = suite("Simple AttrMap suite")(
    ...
    test("AttrMap of a Chunk field values equals a Map of AttributeValue.List") {
    ...
    test("AttrMap of a Vector field values equals a Map of AttributeValue.List") {
      val attrMap = AttrMap(("f1", Vector("s")))
      assert(attrMap.map)(Assertion.equalTo(ScalaMap("f1" -> AttributeValue.List(Chunk(AttributeValue.String("s"))))))
    },
    test("AttrMap of a List field values equals a Map of AttributeValue.List") {
      val attrMap = AttrMap(("f1", List("s")))
      assert(attrMap.map)(Assertion.equalTo(ScalaMap("f1" -> AttributeValue.List(Chunk(AttributeValue.String("s"))))))
    },
    test("AttrMap of a nested AttrMap numeric field values equals a Map of AttributeValue.Map") {
    ...
    test("AttrMap of an Int field values equals a Map of AttributeValue.Number") {
      val attrMap = AttrMap("f1" -> 1)
      assert(attrMap.map)(Assertion.equalTo(ScalaMap("f1" -> AttributeValue.Number(BigDecimal(1.0)))))
    },
    test("AttrMap of a Long field values equals a Map of AttributeValue.Number") {
      val attrMap = AttrMap("f1" -> 1L)
      assert(attrMap.map)(Assertion.equalTo(ScalaMap("f1" -> AttributeValue.Number(BigDecimal(1.0)))))
    },
    test("AttrMap of a Double field values equals a Map of AttributeValue.Number") {
      val attrMap = AttrMap("f1" -> 1.0d)
      assert(attrMap.map)(Assertion.equalTo(ScalaMap("f1" -> AttributeValue.Number(BigDecimal(1.0)))))
    },
    test("AttrMap of a Float field values equals a Map of AttributeValue.Number") {
      val attrMap = AttrMap("f1" -> 1.0f)
      assert(attrMap.map)(Assertion.equalTo(ScalaMap("f1" -> AttributeValue.Number(BigDecimal(1.0)))))
    },
    test("AttrMap of a BigDecimal field values equals a Map of AttributeValue.Number") {
      val attrMap = AttrMap("f1" -> BigDecimal(1.0))
      assert(attrMap.map)(Assertion.equalTo(ScalaMap("f1" -> AttributeValue.Number(BigDecimal(1.0)))))
    },
    test("AttrMap of a number set field values equals a Map of AttributeValue.NumberSet") {
