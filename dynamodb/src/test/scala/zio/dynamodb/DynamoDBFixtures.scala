...
trait DynamoDBFixtures {
  ...
  val getItemT1_2: GetItem         = GetItem(key = primaryKeyT1_2, tableName = tableName1)
  val getItemT1_NotExists: GetItem = GetItem(key = PrimaryKey("k1" -> "NOT_EXISTS"), tableName = tableName1)
  val getItemT3: GetItem           = GetItem(key = primaryKeyT3, tableName = tableName3)
  val getItemT3_2: GetItem         = GetItem(key = primaryKeyT3_2, tableName = tableName3)
