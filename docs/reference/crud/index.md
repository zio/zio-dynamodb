...
There's no High-Level wrapper for batch ops, transact ops, or table management, for two
different reasons:

- **Batch and table management are deliberately Low-Level only.** See
  [Batch Operations](batch.md#why-no-high-level-batch-api) for why batch's partial-success
  shape is best left to the caller to decide on, and [Limitations](../limitations.md) for the
  general FP-modeling-only shape of the High-Level API (table DDL doesn't map onto a "your
  model type" the way item operations do).
- **Transactions simply haven't been built yet** for the High-Level API — see
  ...
  workaround and why today's `get`/`put`/`update`/`deleteFrom` values can't be passed to
  `transactWriteItems` directly.
